...
const zh: Messages = {
  ...
  releaseAnnouncementItems: [
    "修正被動條件判定：人數不足時不再誤顯示「發動」（如菈米需 2 名 5 期生）",
    "修正被動分數加成有時 0% 但全員被動顯示滿足的計算錯誤",
    "修正轟はじめ等主動技能未出現在時間軸的問題",
    "★5 技能秒數以角色名片為準（如風真 30s／11s）",
  ],
...
const en: Messages = {
  ...
  releaseAnnouncementItems: [
    ...
    "★5 skill timing from character-card catalog (e.g. Iroha 30s / 11s)",
  ],
...
const ja: Messages = {
  ...
  releaseAnnouncementTitle: "8/12 更新",
  releaseAnnouncementItems: [
    "パッ条件判定を修正：人数不足で誤って「発動」表示しない（例：ラミィは5期生2名必要）",
    "パッ分数0%なのに全員発動表示になる計算ミスを修正",
    "轟はじめ等のアクティブが時間軸に出ない問題を修正",
    "★5スキル秒数を角色名片準拠に（例：いろは 30s／11s）",
  ],
