...
const zh: Messages = {
  ...
  updateNotesItems: [
    ...
    "★5 資料改為唯一來源：OneDrive Excel 總表（三圍、SP/A/P 技能、隊長衣裝）；build 不再從角色名片 OCR 覆寫",
    "新卡流程：角色名片 OCR 後自動追加至 Excel 並同步（已補 みこ／すいせい 泳裝 ★5）",
  ],
  releaseAnnouncementTitle: "8/13 更新",
  releaseAnnouncementItems: [
    "★5 以 Excel 總表為準；新卡 OCR 後會自動追加至 Excel 再同步",
    "已從角色名片補上 2 張缺漏 ★5（みこ 泳裝、すいせい 夏活）",
  ],
...
const en: Messages = {
  ...
  updateNotesItems: [
    ...
    "★5 data single source: OneDrive Excel master sheet (stats, skills, outfits); build no longer overwrites from card-catalog OCR",
    "New cards: OCR → auto-append to Excel → sync (added missing Miko / Suisei ★5 swimsuit rows)",
  ],
  releaseAnnouncementTitle: "Aug 13 update",
  releaseAnnouncementItems: [
    "★5 from Excel master sheet; new card OCR auto-appends to Excel then syncs",
    "Filled 2 missing ★5 from character cards (Miko beach, Suisei summer)",
  ],
...
const ja: Messages = {
  ...
  updateNotesItems: [
    ...
    "★5データをOneDrive Excel総表のみに統一（ステ・スキル・衣装）；build時に角色名片OCRで上書きしない",
    "新カード：OCR→Excel自動追記→同期（みこ／すいせい水着★5を補完）",
  ],
  releaseAnnouncementTitle: "8/13 更新",
  releaseAnnouncementItems: [
    "★5はExcel総表準拠；新カードOCR後にExcelへ自動追記して同期",
    "角色名片から欠落★5を2枚追加（みこ水着・すいせい夏）",
  ],
