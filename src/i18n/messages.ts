...
const zh: Messages = {
  ...
  releaseAnnouncementItems: [
    "★5 編隊計算改為只讀 OneDrive Excel 總表（三圍、技能、衣裝），不再被 OCR 解析覆蓋",
    "修正被動條件判定與分數加成計算錯誤",
    "修正轟はじめ等主動技能時間軸空白",
  ],
...
const en: Messages = {
  ...
  releaseAnnouncementItems: [
    "★5 team calc now reads only the OneDrive Excel master sheet — no OCR overwrite on build",
    "Fixed passive condition checks and score bonus miscalculation",
...
const ja: Messages = {
  ...
  releaseAnnouncementTitle: "8/13 更新",
  releaseAnnouncementItems: [
    "★5編成計算をOneDrive Excel総表のみ参照（OCR上書き廃止）",
    "パッ条件判定・分数加算の計算ミスを修正",
